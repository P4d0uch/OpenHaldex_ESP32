...
void setUpUI()
{
    ...
    // --- Set mode API ---
    server.on("/api/setMode", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL,
    ...
    server.on("/api/setCustomMode", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL,
              [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
              {
        DynamicJsonDocument doc(2048);
        deserializeJson(doc, (const char*)data);
    ...
    // --- Utility: helper for adding frames ---
    auto addFrame = [](JsonArray arr, const can_frame &f) {
        JsonObject obj = arr.createNestedObject();
        obj["id"] = f.id;
        obj["len"] = f.len;
        obj["timestamp"] = (uint32_t)f.timestamp;
        char dataStr[3 * 8 + 1] = {0};
        for (byte i = 0; i < f.len && i < 8; i++) {
            sprintf(&dataStr[strlen(dataStr)], "%02X ", f.data.bytes[i]);
        }
        obj["data"] = dataStr;
    };

    // --- Haldex CAN frames only ---
    server.on("/api/haldexHistory", HTTP_GET, [addFrame](AsyncWebServerRequest *request)
              {
    ...
    // --- Body CAN frames only ---
    server.on("/api/bodyHistory", HTTP_GET, [addFrame](AsyncWebServerRequest *request)
              {
        DynamicJsonDocument doc(20000);
